...
export function ScrollRail() {
  ...
  return (
    <aside className="pointer-events-none fixed right-6 top-1/2 -translate-y-1/2 z-30 hidden w-44 md:block">
      <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-3">
        <span className="font-mono text-[9px] tracking-[0.3em] text-waran-gold uppercase">TRAVERSE</span>
        <span className="font-mono text-[9px] tracking-widest text-waran-paper/40">
        ...
        {scenes.map((scene) => {
          ...
          return (
            <li key={scene.id} className="flex items-center justify-end gap-2">
              {isActive && (
                <span className="h-1 w-1 rounded-full bg-waran-gold shadow-[0_0_8px_#C5A059] animate-pulse" />
              )}
              <button
                type="button"
                className={cn(
                  ...
                  isActive
                    ...
                    : "text-waran-paper/35 hover:text-waran-paper/70 hover:translate-x-[-2px] opacity-60"
                )}
                onClick={() => {
